...
<script>
    ...
    export default {
        ...
        created() {
            this.api_token = this.get_session_key("tools_token");
        },
        mounted() {
            var _this = this;
            ...
            let vm = this;
            //加载editormd
            vm.articleTxt = _this.articleTxt;
            console.log(vm.articleTxt);
            this.requireEditor(function () {
                vm.editor = editormd(vm.editor_area, {
                    width: "100%",
                    ...
                    onload: function () {
                        this.setMarkdown(_this.articleTxt);
                    }
                });
            });
        },
        mixins: [token, editor],
        methods: {
            async get_article_by_id() {
                var _this = this;
                var res = await this.axios.get(
                    "/api/v1/note_article/?id=" + this.$route.query.id,
                    {
                        headers: {
                ...
                _this.articleCategory = res.data[0].category;
                _this.articleTitle = res.data[0].title;
                _this.articleTxt = res.data[0].body;
                _this.articleId = res.data[0].id;
            },
            /**
             * 异步加载editormd
             * callback 成功后的回调函数
                ...
                if (this.articleTitle == "") {
                    ...
                    this.axios
                        .post(
                            "/dj_note/edit/",
                            "article_id=" +
                            encodeURIComponent(this.articleId) +
                            "&class_id=" +
